...
namespace conversions
{
    ...
    for ind in range(1, len(fields)): # skip the 0th element, which is the base struct
        ...
        if typ != "STRUCT":
            if "[" in typ: # Assuming from empirical evidence that array types have '[' in the type names
                cog.outl("//SKIPPING ARRAY TYPE ASSIGNMENT FOR NOW FOR {}".format(var_name))
            else:
  ...
  //SKIPPING ARRAY TYPE ASSIGNMENT FOR NOW FOR pose_covariance
  ...
  //SKIPPING ARRAY TYPE ASSIGNMENT FOR NOW FOR twist_covariance
