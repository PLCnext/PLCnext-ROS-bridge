...
private:
  ...
  std::vector<rclcpp::TimerBase::SharedPtr> pub_timers_;   /// Contain spawned publisher timers to hold them in scope
  std::vector<typename rclcpp::Publisher<T>::SharedPtr> pubs_;   /// Contain spawned publishers to hold them in scope
  std::vector<typename rclcpp::Subscription<T>::SharedPtr> subs_;   /// Contain spawned subscribers to hold them in scope
...
void BridgeType<T>::getPortParams()
{
  ...
  this->declare_parameter("publishers.datapaths");   /// Array of corresponding instance paths in PLC GDS for the varaible to write to
  this->declare_parameter("publishers.frequencies");   /// Array of corresponding publisher frequencies
  this->declare_parameter("subscribers.topics");   /// Array of subscriber topic names
  this->declare_parameter("subscribers.datapaths");   /// Array of corresponding instance paths in PLC GDS for the varaible to write to
