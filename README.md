...
There are two main parsing python scripts developed in this bridge. These parsers are used in code generation, and are hardcoded to use the IDF in the default directory only - `phoenix_bridge/config/interface_description.yaml`. They can also be directly run from the terminal to see a preview of what kind of data they parse. To do so, you must first cd into the `phoenix_bridge` directory inside this package.

* `phoenix_bridge/phoenix_bridge/param_parser.py`: Offers functions to read the contents of the yaml file to extract the ROS msg types that need to be bridged. Used in all generated sections. Previews the parameter model parsed from the IDF.
* `phoenix_bridge/phoenix_bridge/msg_parser.py`: Offers functions that parse ROS msg types to extract their underlying dictionary structure. Used mainly to generate conversion functions to transform data between grpc type and ROS msg type. Previews either the write or the read conversion code to generate, selectable with the flag `--type, -t` while directly running.

To preview, from the workspace folder:
```
cd src/phoenix_bridge/phoenix_bridge
python3 phoenix_bridge/param_parser.py
python3 phoenix_bridge/msg_parser.py -t write
```

### Liveliness Check

The bridge packs in a [livliness check node](phoenix_bridge/src/liveliness_check.cpp), that pings a variable on the PLC at a determined frequency in a handshake fashion and shouts a warning if check fails. All parameters are [configurable](phoenix_bridge/config/liveliness_config.yaml).
...
CMakeLists.txt has a [custom target](phoenix_bridge/CMakeLists.txt#L29) to generate the code before the package is built.
...
1. Build the project: `colcon build --symlink-install --event-handlers=console_direct+`
...
Unit tests are added using [gtest](https://github.com/google/googletest) under the `phoenix_bridge/test` folder and declared in cmake as well.
